...
import time
from AlgorithmKernel.FitnessBasedWeight import WeightfromMax, WeightfromMini
from AlgorithmKernel.RandomWeight import RandomWeight


# For max objective function

class EGWO:  # Enhanced Grey Wolf Optimizer
    def __init__(self, weight_mode=3, variance_mode=1):
        # wa>wb>wd
        # sum(wa,wb,wd)=1
        self.weight_mode = weight_mode
        self.var_mode = variance_mode
    ...
    def EGWO(self, objf, lb, ub, dim, SearchAgents_no, Max_iter, add_random=True):
        ...
        :param Max_iter:
        :param add_random:
        :param weight_mode: weight generate way: 1.fixed 2.random 3.fitness
        :return:
        """
        ...
        for l in range(0, Max_iter):
            ...
            # first generate random nums if mode==2or3
            if self.weight_mode == 2:
                self.wa, self.wb, self.wd = RandomWeight([Alpha_score, Beta_score, Delta_score])
            elif self.weight_mode == 3:
                self.wa, self.wb, self.wd = WeightfromMax([Alpha_score, Beta_score, Delta_score])
            if add_random == True:
                if self.var_mode == 1:
                    var = (1 - ((1 * l) / Max_iter))
                elif self.var_mode == 2:
                    var = (1 - ((1 * l) / Max_iter) ** 2)
                elif self.var_mode == 3:
                    var = math.exp(-100 * ((1 * l) / Max_iter))
        ...
        para_setting = {"实验编号": ["3"], "狼群规模": [str(SearchAgents_no)], "权重生成方式": [str(self.weight_mode)],
                        "alpha weight": [str(self.wa)], "beta weight": [str(self.wb)], "delta weight": [str(self.wd)],
                        "标准差下降方式": [str(self.var_mode)]}
